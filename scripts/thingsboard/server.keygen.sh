...
usage() {
 ...
 echo " -c | --copy flag Specifies if the keystore should be copied to the server
directory. Defaults to true"
 echo " -d | --dir directory Server keystore directory, where the generated keystore file
will be copied. If specified, overrides the value from the properties file"
 echo " Default value is SERVER_KEYSTORE_DIR property from
properties file"
...
while :
 do
 read -p "Output files from previous server.keygen.sh script run found. Overwrite?[yes]"
response
 case $response in
 [nN]|[nN][oO])
 echo "Skipping"
...
keytool -genkeypair -v \
 ...
 -dname "CN=$DOMAIN_SUFFIX, OU=$ORGANIZATIONAL_UNIT, O=$ORGANIZATION,
L=$CITY, ST=$STATE_OR_PROVINCE, C=$TWO_LETTER_COUNTRY_CODE" \
 ...
 -keysize 2048 \
 -validity 9999 \
 -ext SAN=dns:mcastello.dyndns.org,ip:192.168.1.19
status=$?
if [[ $status != 0 ]]; then
 exit $status;
fi
...
if [[ $COPY = true ]]; then
 ...
 while :
 do
 read -p "Do you want to copy $SERVER_FILE_PREFIX.jks to server directory? [Y/N]: "
yn
 case $yn in
 [nN]|[nN][oO])
 break
